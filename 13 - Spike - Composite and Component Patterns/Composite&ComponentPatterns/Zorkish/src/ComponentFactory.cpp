...
*		<ComponentFlag, "Args, ..., Args">>
*/
std::vector<std::vector<std::string>> ComponentFactory::formatComponentData() {
	std::vector<std::vector<std::string>> formatted_dataset;
	...
	if (!_raw_component_dataset.empty()) {
		...
		for (std::string& component : _raw_component_dataset) {
			size_t flag_start_idx = 0,
				   flag_end_idx = component.find_first_of('(') - 1,
				   args_start_idx = component.find_first_of('(') + 1,
				   args_end_idx = component.find_first_of(')') - 1;
...
std::vector<std::string> ComponentFactory::formatArgs
	...
	for (auto& component_datum : component_data) {
...
void ComponentFactory::createComponent(int entity_id, 
	std::vector<std::string>& component_data) {
	ComponentFlag flag = determineFlag(component_data[0]);
	component_data.erase(component_data.begin());
	component_data.shrink_to_fit();

	if (flag == ComponentFlag::SPATIAL) {
		C_Spatial* new_spatial = new C_Spatial(entity_id);
		_game_data->c_spatials.insert({ entity_id, new_spatial });
		return;
	}
	
	std::vector<std::string> component_args = formatArgs(component_data);
	...
	*	exist as entities with rendered names which can be referenced		*/
	if (flag == ComponentFlag::INVENTORY) { 
			...
			for (std::string item_name : component_args) {
				for (auto idx = _game_data->c_renderers.begin();
					...
					if (idx->second->getName() == item_name) {
						...
						auto it = _game_data->entities.find(entity_id);

						InventorySlot* new_slot = new InventorySlot(it->second);
...
void ComponentFactory::constructComponents(int entity_id) {
	...
	if (!_raw_component_dataset.empty()) {
		_fmt_component_dataset = formatComponentData();

		for (auto& component_data : _fmt_component_dataset) {
			createComponent(entity_id, component_data);	}
	}
}
