...
WorldLoader::~WorldLoader() {
	...
	for (auto& location : _location_data) {
		delete location;
		location = nullptr;
	}
...
void WorldLoader::instanceWorldData() {
	for (auto location : _prefmt_world_data) {
		...
		_location_data.emplace_back(new_location);
	}
...
void WorldLoader::wireLocationExits() {
	...
	for (int loc_idx = 0; loc_idx < _location_data.size(); loc_idx++) {
		...
		for (int datum_idx = 0; datum_idx < raw_dir_exit_data.size();
			datum_idx += 2) {
			_location_data[loc_idx]->addExit(raw_dir_exit_data[datum_idx],
...
/*****************************************************************************
*   Public:																	*/
GameData* WorldLoader::loadWorldData() {
	...
	if (_reader.is_open()) {
		...
		game_data->locations = _location_data;
		game_data->current_location = game_data->locations[0];
